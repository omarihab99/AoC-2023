...
fn main() {
    ...
    for (i, line) in input.iter().enumerate() {
        ...
        for (j, c) in line.chars().enumerate() {
            part_one(&mut valid, &mut number, &input, &c, i, j, &mut sum, &line);
        }
        // This check is required to solve part one of the problem.
        if valid {
            sum += number;
        }
    }
...
fn part_one(
    ...
    if c.is_digit(10) {
        ...
        if i >= 1 && j >= 1 {
            ...
            *valid |=
                left_up.is_some() && !left_up.unwrap().is_digit(10) && left_up.unwrap() != '.';
        }
        if i >= 1 && j + 1 < line.len() {
            let left_down = input[i - 1].chars().nth(j + 1);
            *valid |= left_down.is_some()
                && !left_down.unwrap().is_digit(10)
                && left_down.unwrap() != '.';
        }
        if i + 1 < input.len() && j >= 1 {
            let right_up = input[i + 1].chars().nth(j - 1);
            *valid |=
                right_up.is_some() && !right_up.unwrap().is_digit(10) && right_up.unwrap() != '.';
        }
        if i + 1 < input.len() && j + 1 < line.len() {
            let right_down = input[i + 1].chars().nth(j + 1);
            *valid |= right_down.is_some()
                && !right_down.unwrap().is_digit(10)
                && right_down.unwrap() != '.';
        }
